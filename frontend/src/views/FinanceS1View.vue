        ...
        <section class="result-panel">
          ...
          <div v-if="activeTab === 'overview'" class="panel-content overview-view">
            <div class="progress-block">
              ...
              <div class="direction-summary"><span>历史记忆 {{ mapping?.history_memory?.length || 0 }} 条</span><span>互动 {{ manifest?.social_rounds || socialRounds }} 轮</span><span>Pre / Post 各 {{ investorCount }} 次预测</span></div>
            </div>
          ...
          <div v-else-if="activeTab === 'results'" class="panel-content result-view">
            ...
            <div v-if="hasRun && predictions.length" class="csv-actions"><a :href="csvUrl('predictions')">下载两阶段预测</a><a :href="csvUrl('agent_changes')">下载 Agent 变化</a><a v-if="manifest?.status === 'completed'" :href="csvUrl('evaluation')">下载评测</a><a v-if="manifest?.status === 'completed'" :href="csvUrl('agent_token_usage')">下载 Agent Token</a></div>
...
const investorCount = computed(() => manifest.value?.investor_agent_count || 10)
const expectedCount = computed(() => manifest.value?.expected_prediction_count || investorCount.value * 2)
...
const progressTitle = computed(() => manifest.value?.status === 'completed' ? `${investorCount.value} 个 Agent 的两阶段预测已完成配对` : manifest.value?.status === 'running' ? `正在执行：${phaseLabel(manifest.value.current_phase)}` : '等待启动社会互动实验')
